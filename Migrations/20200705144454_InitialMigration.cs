...
namespace TopRecords.Migrations
{
    public partial class InitialMigration : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.CreateTable(
               ...
               constraints: table =>
               {
                   table.PrimaryKey("PK_TopRecords", x => new { x.ID });
               });

            migrationBuilder.Sql("CREATE proc [dbo].[spGetRecords] " +
                " as BEGIN  " +
                "SELECT * from dbo.TopRecords order by Time " +
                "END");

            migrationBuilder.Sql("CREATE proc [dbo].[spAddRecord](@name varchar(50), @surname varchar(50), @email varchar(254), @time time(0))" +
                " as BEGIN " +
                    "Insert into dbo.TopRecords(Name, Surname, Email, Time, Approved)" +
                    " values(@name, @surname, @email, @time, 0) " +
                "END");

            migrationBuilder.Sql("CREATE proc [dbo].[spUpdateRecord](@Id int, @isApproved bit)" +
                " as BEGIN " +
                "if @isApproved = 0 " +
                    "Delete from dbo.TopRecords where ID = @Id " +
                "else Update dbo.TopRecords " +
